...
def dialogue_segmentation(logs, selection, seg_verbose=False):
    ...
    :param selection: list of dicts. In each dict, the only key is the game
    id, the values are the rounds to include in this split.
    ...
    dialogue_sections = []
    games_to_include = [int(list(entry.keys())[0]) for entry in selection]
    roundlookup = dict(ChainMap(*selection))

    for game in logs:
        game_id = game.game_id
        if selection and game_id not in games_to_include:
            continue
        rounds_to_include = roundlookup[str(game_id)]
...
def game_segmentation(game, seg_verbose, cleaning_total, section_counter,
    ...
    for round_id in rounds_to_include:
        round_data = game.rounds[round_id]
...
if __name__ == '__main__':
    ...
    with open(os.path.join(data_path, "analysis_data_splits.json"), 'r') as f:
    ...
    for set_name in ['valid', 'test', 'train']:
        set_dicts = data_split[set_name]
        dialogue_sections = dialogue_segmentation(
            logs, set_dicts, seg_verbose=False)
        with open(os.path.join(data_path, "analysis_{}_sections.pickle".format(set_name)), 'wb') as f:
